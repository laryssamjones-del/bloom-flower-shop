...
export interface ShelfNPCCustomizationConfig {
  height: number; // px
...
export function loadShelfNPCCustomizationConfig(): ShelfNPCCustomizationConfig {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (raw) {
      const parsed = JSON.parse(raw) as ShelfNPCCustomizationConfig;
      return parsed;
    }
  } catch {}
...
export function ShelfNPCCustomizer({ onClose }: Props) {
  const storedConfig = useGameStore((s) => s.shelfNPCCustomizationConfig);
  const [config, setConfig] = useState<ShelfNPCCustomizationConfig>(() =>
    storedConfig || loadShelfNPCCustomizationConfig()
  );
  const [draggingNPC, setDraggingNPC] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);
  const npcDragStartRef = useRef({ x: 0, y: 0, offsetX: 0, offsetY: 0 });
  const saveShelfNPCCustomizationConfig = useGameStore((s) => s.saveShelfNPCCustomizationConfig);
  ...
  const handleSave = () => {
    ...
    saveShelfNPCCustomizationConfig({
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: '10px', color: '#777', textAlign: 'center' }}>
          Adjust positioning for NPCs buying from shelf · drag NPC to reposition
        </div>
